...
- _Result_: `Log Storage` success drops to 0% -> `Checkout Server` (which depends on logs) sees its success drop to 0% -> The user-facing "Success" metric on the dashboard crashes, even if the `Checkout Server` GCU/RAM are perfectly healthy.
