...
<main class="game-container">
	<header class="global-header">
		...
		<div class="controls">
			<button class="btn {isRunning ? 'stop' : 'start'}" onclick={() => isRunning ? engine.stop() : engine.start()}>
				{isRunning ? 'PAUSE' : 'RUN'}
			</button>
		</div>
	</header>
	...
	<div class="content">
		<nav class="side-nav">
			<button class="nav-item {activeView === 'dashboard' ? 'active' : ''}" onclick={() => activeView = 'dashboard'}>Dashboard</button>
			<button class="nav-item {activeView === 'actions' ? 'active' : ''}" onclick={() => activeView = 'actions'}>Actions</button>
...
<style>
	...
	.version { font-size: 0.6rem; color: #555; }
	...
	.stat-item .label { font-size: 0.6rem; color: #666; }
	.stat-item .value { font-size: 1.1rem; font-weight: 700; color: #4ade80; }
	.low-budget .value { color: #f87171; }
	...
	.btn.start { color: #4ade80; border-color: #4ade8055; }
